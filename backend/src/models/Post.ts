...
postSchema.pre('save', function () {
    if (this.isModified('title') || !this.slug) {
        ...
        // Append short random string or timestamp to ensure uniqueness
        const uniqueId = Math.random().toString(36).substring(2, 7);
        this.slug = `${baseSlug || 'post'}-${uniqueId}`;
    }
...
postSchema.methods.getAnswerScore = function (answerId: any) {
...
postSchema.methods.canEdit = function (userId: any) {
